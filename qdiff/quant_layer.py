...
class QuantModule(nn.Module):
    ...
    def forward(self, input: torch.Tensor, split: int = 0):
        ...
        # skip it!
        #if self.nametag == 'model.time_extra_emb.timestep_embedder.linear_1':
            #print("====" + self.nametag + "=====")
            #return self.org_module(input.to(torch.float16))
        ...
        #print(weight.dtype)
        #print(input.dtype)
        #If the type is not match, just skip it! But print to the logger that we skipped this layer's quantization
        #print("Dunno")
        #print(input.device)
        #print(weight.device)
        #print(bias.device)
        out = self.fwd_func(input, weight.to(input.device), bias.to(input.device), **self.fwd_kwargs)
