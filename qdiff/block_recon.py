...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, cali_data: torch.Tensor,
    ...
    set_back_to_0 = False
    if block.go_back_to_0:
        block.go_back_to_0 = False
        set_back_to_0 = True        
    ...
    if optimizer is not None:
        for i in range(iters):
            if isinstance(cached_inps, list):
                ...
                if len(cached_inps) == 5:
                    cur_x = cached_inps[0][idx].to(device, torch.float32)
                    cur_ehs = cached_inps[1][idx].to(device, torch.float32) if type(cached_inps[1]) == torch.Tensor else None
                    cur_t = cached_inps[2][idx].to(device, torch.float32) if type(cached_inps[2]) == torch.Tensor else None
                    cur_ire = None
                    cur_s = cached_inps[4][idx].to(device, torch.float32) if type(cached_inps[4]) == torch.Tensor else None
                    cur_inp = (cur_x, cur_ehs, cur_t, cur_ire, cur_s)
                else:
    ...
    block.go_back_to_0 = set_back_to_0
