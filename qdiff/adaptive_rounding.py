...
class AdaRoundQuantizer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.round_mode == 'nearest':
            ...
            if self.fp == False:
                x_floor = torch.floor(x / self.delta.to(x.device))
                if self.soft_targets:
                    x_int = x_floor + self.get_soft_targets().to(x.device)
                else:
                    x_int = x_floor + (self.alpha.to(x.device) >= 0).float()
            else:
                if self.soft_targets:
                ...
                # NOTE for group quantization
                if self.group_quant == True:
                    x_dequant = x_dequant.reshape(x_old.shape)
                return x_dequant
        else:
            raise ValueError('Wrong rounding mode')

        x_quant = torch.clamp(x_int + self.zero_point.to(x.device), 0, self.n_levels - 1)
        x_float_q = (x_quant - self.zero_point.to(x.device)) * self.delta.to(x.device)
    ...
    def get_soft_targets(self):
        ...
        if (self.fp == True) and (self.fp_biased_adaround == True):
            return torch.clamp(torch.sigmoid(self.alpha.to(self.dev) / self.scale.to(self.dev)) * (self.zeta - self.gamma) + self.gamma, 0, 1)
        return torch.clamp(torch.sigmoid(self.alpha.to(self.dev)) * (self.zeta - self.gamma) + self.gamma, 0, 1)
