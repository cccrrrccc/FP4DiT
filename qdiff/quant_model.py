...
class QuantModel(nn.Module):
    ...
    def __init__(self, model: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}, **kwargs):
        ...
        self.model = model
        #self.model = FullyShardedDataParallel(model, #fsdp_auto_wrap_policy=default_auto_wrap_policy, 
    ...
    #def shard_model(self):
        for i in range(15):
            print(0)
            self.model.blocks[i].to("cuda:0")
            self.model.blocks[i].c_str = "cuda:0"
            self.model.blocks[i].hack_dev = 0
            self.model.blocks[i].go_back_to_0 = False
        for i in range(15, 40):
            print(1)
            self.model.blocks[i].to("cuda:1")
            self.model.blocks[i].c_str = "cuda:1"
            self.model.blocks[i].hack_dev = 1
            self.model.blocks[i].go_back_to_0 = False
        ...
        self.model.blocks[-1].go_back_to_0 = True
    ...
    def set_ff1_weight_mantissa_bits(self, weight_quant_params: dict = {}):
        ...
        for m in self.model.modules():
            ...
            if isinstance(m, QuantHunyuanBlock):
                m.ff.net[0].proj.weight_quantizer.mantissa_bits[0] = num
    ...
    def quant_module_refactor(self, module: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}):
        ...
        for name, child_module in module.named_children():
            ...
            # NOTE: this is to skip this module as its dtype problem
            # To remove this, address the problem in quant_layer.py L569
            if name == "time_extra_emb":
                continue

            # NOTE: for simplification just quant the DiT layer
            if name == "pos_embed":
                continue
            if name == "text_embedder":
                continue
            if name == "norm_out":
                continue
            if name == "proj_out":
