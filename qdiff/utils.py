...
from qdiff.quant_block import BaseQuantBlock, QuantDiffBTB, QuantDiffRB, QuantHunyuanBlock
...
def save_inp_oup_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor,
    ...
    elif len(cali_data) == 6:
        cali_xs, cali_ts, cali_conds1, cali_conds2, cali_mask1, cali_mask2 = cali_data # Here; Sep 17
        cali_ts = cali_ts.to(torch.float16) # By Ruichen
    else:
    ...
    for i in trange(num):
        if not cond:
            ...
            # cali_xs, cali_ts, cali_conds1, cali_conds2, cali_mask1, cali_mask2
            cur_inp, cur_out = get_inp_out(
                cali_xs[i * batch_size:(i + 1) * batch_size].to(device), 
                cali_ts[i * batch_size:(i + 1) * batch_size].to(device),
                cali_conds1[i * batch_size:(i + 1) * batch_size].to(device),
                cali_conds2[i * batch_size:(i + 1) * batch_size].to(device),
                cali_mask1[i * batch_size:(i + 1) * batch_size].to(device),
                cali_mask2[i * batch_size:(i + 1) * batch_size].to(device)
            ) if not is_sm else get_inp_out(
                cali_xs[i * batch_size:(i + 1) * batch_size].to(device), 
                cali_ts[i * batch_size:(i + 1) * batch_size].to(device),
                cali_conds1[i * batch_size:(i + 1) * batch_size].to(device),
    ...
    if not is_sm:
        ...
        # add self.data_saver.input_store[7] len should be 7 -> 8 (pixart) -> 5 (hunyuan)
        if isinstance(cached_batches[0][0], tuple) and len(cached_batches[0][0]) == 5:
            '''
            # pixart 8 -> hunyuan 5
            cached_inps = [
            ...
            cached_inps = [
                ...
                torch.cat([x[0][1] for x in cached_batches]) if cached_batches[0][0][1] is not None else [None] * len(cached_batches),
                torch.cat([x[0][2] for x in cached_batches]) if cached_batches[0][0][2] is not None else [None] * len(cached_batches),
                None, # NOTE: torch.cat([x[0][1] for x in cached_batches]) doesn't work
                torch.cat([x[0][4] for x in cached_batches]) if cached_batches[0][0][4] is not None else [None] * len(cached_batches)
            ]
...
class DataSaverHook:
    ...
    def __call__(self, module, input_batch, output_batch):
        if self.store_input:
            ...
            if hasattr(module, "t_cache"):
                self.input_store = (input_batch[0],
                                    module.ehs_cache,
                                    module.t_cache,
                                    module.ire_cache,
                                    module.s_cache)  # add module.added_cond_kwargs
...
class GetLayerInpOut:
    ...
    def __call__(self, x, timesteps, encoder_hidden_states=None, encoder_hidden_states_t5=None, 
        text_embedding_mask=None, text_embedding_mask_t5=None):
        self.model.eval()
        ...
        with torch.no_grad():
            try:
                ...
                #_ =(cali_xs[:2].cuda(), timestep=cali_ts[:2].cuda(), encoder_hidden_states=cali_cs1[:2].cuda(), encoder_hidden_states_t5=cali_cs2[:2].cuda(),
                #   text_embedding_mask=cali_m1[:2].cuda(), text_embedding_mask_t5=cali_m2[:2].cuda(), image_rotary_emb = hunyuan_image_rotary_emb(model.transformer))
                add_time_ids, style = hunyuan_size_and_style(encoder_hidden_states)
                _ = self.model(x, timestep=timesteps, encoder_hidden_states=encoder_hidden_states, encoder_hidden_states_t5=encoder_hidden_states_t5,
                text_embedding_mask=text_embedding_mask, text_embedding_mask_t5=text_embedding_mask_t5, image_rotary_emb=hunyuan_image_rotary_emb(),
        ...
        elif isinstance(self.layer, QuantHunyuanBlock):
            # self.data_saver.input_store[3] is a tuple
            #buffer = []
            #for tensor in self.data_saver.input_store[3]:
            #    buffer.append(tensor.detach().cpu())
            #buffer = tuple(buffer)
            buffer = None
            return (self.data_saver.input_store[0].detach().cpu(),
                    self.data_saver.input_store[1].detach().cpu() if self.data_saver.input_store[1] is not None else None,
                    self.data_saver.input_store[2].detach().cpu() if self.data_saver.input_store[2] is not None else None,
                    buffer,
                    self.data_saver.input_store[4].detach().cpu() if self.data_saver.input_store[4] is not None else None), self.data_saver.output_store.detach().cpu()
...
def get_train_samples_custom_ucs(args, sample_data, custom_steps=None):
    ...
    xs_lst = [sample_data["xs"][i][:num_samples] for i in timesteps]
    ts_lst = [sample_data["ts"][i][:num_samples] for i in timesteps]
    if (args.cond) and ("ucs1" in sample_data):
        xs_lst += xs_lst
        ts_lst += ts_lst
        conds_lst1 = [sample_data["cs1"][i][:num_samples] for i in timesteps] + [sample_data["ucs1"][i][:num_samples] for i in timesteps]
        conds_lst2 = [sample_data["cs2"][i][:num_samples] for i in timesteps] + [sample_data["ucs2"][i][:num_samples] for i in timesteps]
        mask1_lst = [sample_data["m1"][i][:num_samples] for i in timesteps] + [sample_data["um1"][i][:num_samples] for i in timesteps]
        mask2_lst = [sample_data["m2"][i][:num_samples] for i in timesteps] + [sample_data["um2"][i][:num_samples] for i in timesteps]
    xs = torch.cat(xs_lst, dim=0)
    ts = torch.cat(ts_lst, dim=0)
    conds1 = torch.cat(conds_lst1, dim=0)
    conds2 = torch.cat(conds_lst2, dim=0)
    mask1 = torch.cat(mask1_lst, dim=0)
    mask2 = torch.cat(mask2_lst, dim=0)
    return xs, ts, conds1, conds2, mask1, mask2
...
def resume_cali_model(qnn, ckpt_path, cali_data, quant_act=False, act_quant_mode='qdiff', cond=False):
    ...
    # NOTE Exception for SDXL
        ...
        cali_xs, cali_ts, cali_conds1, cali_conds2, cali_mask1, cali_mask2 = cali_data # Here; Sep 17
        cali_ts = cali_ts.to(torch.float16) # By Ruichen
        add_time_ids, style = hunyuan_size_and_style(cali_conds1[:2].cuda())
        _ = qnn(cali_xs[:2].to("cuda:0", dtype=torch.float16), timestep=cali_ts[:2].to("cuda:0", dtype=torch.float16), encoder_hidden_states=cali_conds1[:2].to("cuda:0", dtype=torch.float16), encoder_hidden_states_t5=cali_conds2[:2].to("cuda:0", dtype=torch.float16),
                text_embedding_mask=cali_mask1[:2].to("cuda:0", dtype=torch.float16), text_embedding_mask_t5=cali_mask2[:2].to("cuda:0", dtype=torch.float16), image_rotary_emb=hunyuan_image_rotary_emb(),
                image_meta_size=add_time_ids, style=style)
...
def hunyuan_image_rotary_emb():
    # 7 create image_rotary_emb, style embedding & time ids
    height = 1024
    width = 1024
    #grid_height = height // 8 // pipeline.transformer.config.patch_size (2)
    #grid_width = width // 8 // pipeline.transformer.config.patch_size
    #base_size = 512 // 8 // pipeline.transformer.config.patch_size
    grid_height = height // 8 // 2
    grid_width = width // 8 // 2
    base_size = 512 // 8 // 2
    grid_crops_coords = get_resize_crop_region_for_grid((grid_height, grid_width), base_size)
    #image_rotary_emb = get_2d_rotary_pos_embed(
    #    pipeline.transformer.inner_dim (1048) // pipeline.transformer.num_heads, grid_crops_coords, (grid_height, grid_width)
    #)
    image_rotary_emb = get_2d_rotary_pos_embed(
        1408 // 16, grid_crops_coords, (grid_height, grid_width)
    )
    return image_rotary_emb

def hunyuan_size_and_style(prompt_embeds):
    # NOTE: Default Value, might be incorrect
    original_size = (1024, 1024)
    target_size = (1024, 1024) # NOTE: Only support for 1024 res
    ...
    # NOTE: Don't know if need this
    #if self.do_classifier_free_guidance:
    #    add_time_ids = torch.cat([add_time_ids] * 2, dim=0)
    #    style = torch.cat([style] * 2, dim=0)
    add_time_ids = add_time_ids.to(dtype=prompt_embeds.dtype, device='cuda').repeat(
            batch_size * 1, 1
        )
    style = style.to(device='cuda').repeat(batch_size * 1)

    return add_time_ids, style
