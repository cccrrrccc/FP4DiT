...
def hunyuan_size_and_style(prompt_embeds):
    ...
    style = style.to(device=device).repeat(batch_size * 1)
...
class QuantHunyuanBlock(BaseQuantBlock):
    ...
    def forward(self, 
        ...
        if hidden_states.device.index != self.hack_dev:
            hidden_states = hidden_states.to(self.c_str)
        if encoder_hidden_states.device.index != self.hack_dev:# is not None:
            encoder_hidden_states = encoder_hidden_states.to(self.c_str)
        if temb.device.index != self.hack_dev: # is not None:
            temb = temb.to(self.c_str)
            #if image_rotary_emb is not None:
            #    image_rotary_emb = image_rotary_emb.to(c_str)
        if skip is not None:
            if skip.device.index != self.hack_dev:# is not None:
                skip = skip.to(self.c_str)
    ...
    def _forward(
        ...
        # NOTE: image_rotary_emb
        # NOTE: Don't know how to cache it, generate it everytime I need as it doesn't change
        image_rotary_emb = hunyuan_image_rotary_emb()
