...
def main():
    ...
    parser.add_argument(
        ...
        help="generate images for coco 2014 prompts 0-4999")
    parser.add_argument(
    ...
    model = HunyuanDiTPipeline.from_pretrained("Tencent-Hunyuan/HunyuanDiT-Diffusers", torch_dtype=torch.float16)
    #del model.transformer.blocks[30:39]
    #del model.transformer.blocks[20:29]
    #del model.transformer.blocks[11:20]
    #del model.transformer.blocks[1:10]
    #model.config.num_layers = len(model.transformer.blocks)
    model = model.to("cuda")
    print(f"Num blocks: {len(model.transformer.blocks)}")
    
    #model.transformer.blocks = model.transformer.blocks[:3]
    ...
    if opt.prompt is None:
        ...
        model.text_encoder_2 = model.text_encoder_2.to("cpu")
    ...
    if opt.ptq:
        ...
        if opt.resume:
            ...
            #print(qnn)
            #print(cali_ts[:1].cuda())
            #print(cali_ts[:1].dtype)
            #print(cali_cs1[:1].shape) #torch.Size([2, 77, 1024])
            #print(cali_cs2[:1].shape) #torch.Size([2, 256, 2048])
            ...
            add_time_ids, style = hunyuan_size_and_style(cali_cs1[:1].cuda()) # Input consists with encoder_hidden_states
            _ = qnn(cali_xs[:1].cuda(), timestep=cali_ts[:1].cuda(), encoder_hidden_states=cali_cs1[:1].cuda(), encoder_hidden_states_t5=cali_cs2[:1].cuda(),
            text_embedding_mask=cali_m1[:1].cuda(), text_embedding_mask_t5=cali_m2[:1].cuda(), image_rotary_emb = hunyuan_image_rotary_emb(),
    ...
    #model.text_encoder_2 = model.text_encoder_2.to("cuda")
